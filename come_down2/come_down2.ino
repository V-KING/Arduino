...
//检测按键是否释放的函数
//输入为空；返回为int， 1表示按键释放，0表示未释放
int priorButtonState = HIGH;
int buttonRelease(int button) {
	if(digitalRead(button) == LOW){   //如果读取到按键引脚为低电平，表示按键被按下
		priorButtonState=LOW;	      //把按键状态变量置为低
		return 0;		      //按键没有释放，返回0
	}
	else {			      //如果读取到按键引脚为高电平
		if(priorButtonState==LOW) {	      //如果按键上一状态为低电平，则表示按键被释放
			priorButtonState=HIGH;        //将按键状态变量置为高
			return 1;	              //按键释放，返回1
		}
		else {			      //按键未按下，返回0
			return 0;
...
/*
 * 如果检查到状态buttonActionState，根据状态干事
 */
	...
	switch(lable_beep){
		case 415:
			while(1){
				...
				case 417:
					if(!(beepTime_s==5)){    //<---------beepTime_s==3时就往下走
						return 0;
					}
